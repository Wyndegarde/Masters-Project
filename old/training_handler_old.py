''' Intermediary script to hold training handler class before cleaning up for testing.'''
...
class TrainingHandler:
    ...
    def train_model(self) -> defaultdict:
        ...
        end_time = time.time()
        TrainingServices.print_final_metrics(accuracy, avg_train_loss, valid_accuracy, avg_valid_loss)
        return self.history
    ...
    def _spiking_training_loop(self, train_data, train_labels) -> Tuple[Tensor,Tensor]:
        ...
        # Sum loss over time steps: BPTT
        loss: Tensor = torch.zeros((1), dtype=self.dtype, device=self.device)   # creates a 1D tensor to store total loss over time. 
        for step in range(self.num_steps):
            loss += self.loss_fn(mem_rec[step], spk_targets_it) # Loss at each time step is added to give total loss.
    ...
    def _validation_loop(self) -> Tuple[float,float]:
        ...
        with torch.no_grad():
            for valid_X, valid_y in self.valid_loader:
                ...
                if self.model_type == "spiking":
                    valid_predicted, valid_loss = self._spiking_training_loop(valid_X, valid_y)
                else:
                    # Get model output
                    valid_predicted = self.model(valid_X)

                    # Calculate metrics for each batch
                    valid_loss = self.loss_fn(valid_predicted, valid_y)

                # Calculate average loss
                avg_valid_loss += valid_loss.item()
                valid_correct += (
                    (valid_predicted.argmax(1) == valid_y).type(torch.float).sum().item()
                )
